...
def distribute_work(state: DeformExpertState) -> Union[str, List[Send]]:
    return distribute_work_generic(state, "Deform")
...
def _deform_initial_state_factory(temp_image_path, hotspots, preprocessed_hotspots, **kwargs) -> DeformExpertState:
    return {
        "messages": [],
...
deform_expert_wrapper_node = create_expert_wrapper_node(
    expert_name="Deform",
    expert_id="deform",
    build_graph_fn=build_deform_expert_graph,
    default_initial_state_factory=_deform_initial_state_factory
)

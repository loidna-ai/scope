...
def distribute_work(state: NeckingExpertState) -> Union[str, List[Send]]:
    return distribute_work_generic(state, "Necking")
...
def _necking_initial_state_factory(temp_image_path, hotspots, preprocessed_hotspots, **kwargs) -> NeckingExpertState:
    return {
        "messages": [],
...
necking_expert_wrapper_node = create_expert_wrapper_node(
    expert_name="Necking",
    expert_id="necking",
    build_graph_fn=build_necking_expert_graph,
    default_initial_state_factory=_necking_initial_state_factory
)

...
from src.nodes.experts.mechanical_expert import (
...
TOOL_TO_STEP_KEY = {
    "analyze_mechanical_deformation": "mechanical_step1_result",
    "analyze_strand_splaying": "mechanical_step2_result",
    "analyze_bead_confinement": "mechanical_step3_result"
}
...
def create_step_tools():
    ...
    """
    @tool
    def analyze_mechanical_deformation(image_path: str) -> Dict[str, Any]:
        """기계적 변형 흔적을 분석하여 압착 또는 기계적 손상 여부를 판단합니다."""
        try:
            image_data = _load_image_data(image_path)
            return step1_mechanical_deformation(image_data, verbose=False)
        except Exception as e:
            return {"error": str(e)}
    
    @tool
    def analyze_strand_splaying(image_path: str) -> Dict[str, Any]:
        """연선의 소선 배열 흐트러짐을 분석하여 강제 변형 여부를 확인합니다."""
        try:
            image_data = _load_image_data(image_path)
            return step2_strand_splaying(image_data, verbose=False)
        except Exception as e:
            return {"error": str(e)}
    
    @tool
    def analyze_bead_confinement(image_path: str) -> Dict[str, Any]:
        """단락흔(용융망울)의 위치와 구속 상태를 분석하여 물리적 구속 여부를 판단합니다."""
        try:
            image_data = _load_image_data(image_path)
            return step3_bead_confinement(image_data, verbose=False)
        except Exception as e:
            return {"error": str(e)}
    
    return [
        analyze_mechanical_deformation,
        analyze_strand_splaying,
        analyze_bead_confinement
    ]
...
def create_agent_node(all_tools: List[Any]):
    """ReAct 에이전트 노드 생성"""
    def agent_node(state: MechanicalExpertState) -> Dict[str, Any]:
...
당신은 전기화재 감식 전문가 'Mechanical Expert'입니다.
...
2. 상황에 따라 필요한 도구를 자유롭게 선택하여 사용하세요. 사용 가능한 분석 도구:
   - analyze_mechanical_deformation: 기계적 변형 흔적 분석 (압착 또는 기계적 손상 여부 판단)
   - analyze_strand_splaying: 연선의 소선 배열 흐트러짐 분석 (강제 변형 여부 확인)
   - analyze_bead_confinement: 단락흔(용융망울)의 위치와 구속 상태 분석 (물리적 구속 여부 판단)
...
def build_mechanical_expert_graph():
    """
    Mechanical 전문가 서브그래프 빌드
    
    그래프 구조: START → agent → [조건부: tools_condition → tools 또는 종료] ← tools ───┘
    """
    builder = StateGraph(MechanicalExpertState)
...
def mechanical_expert_wrapper_node(state: InvestigationState) -> Dict[str, Any]:
    """
    InvestigationState를 MechanicalExpertState로 변환하여 ReAct 그래프를 실행하고,
    ...
    try:
        ...
        if image_data is None:
            return {
                "errors": ["Mechanical 전문가: 이미지를 추출할 수 없습니다."],
                "expert_reports": [],
        ...
        initial_state: MechanicalExpertState = {
            "messages": [
                HumanMessage(content=f"이미지를 분석하고 압착 또는 기계적 손상 여부를 판단하세요. 이미지 경로: {temp_image_path}")
            ],
            "image_path": temp_image_path,
            "mechanical_step1_result": None,
            "mechanical_step2_result": None,
            "mechanical_step3_result": None
        }
        
        graph = build_mechanical_expert_graph()
        final_state = graph.invoke(initial_state, config={"recursion_limit": 100})
        
        messages = final_state.get("messages", [])
        step_results = {
            "mechanical_step1_result": {},
            "mechanical_step2_result": {},
            "mechanical_step3_result": {}
        }
        ...
        step1_result = step_results["mechanical_step1_result"]
        step2_result = step_results["mechanical_step2_result"]
        step3_result = step_results["mechanical_step3_result"]
        ...
        return {
            ...
            "expert_confidence_scores": {"mechanical": confidence_score},
            "expert_evidence": {"mechanical": evidence},
            **step_results
        }
    except Exception as e:
        import traceback
        traceback.print_exc()
        return {
            "errors": [f"Mechanical 전문가 ReAct 에이전트 오류: {str(e)}"],
            "expert_reports": [],
            "expert_analysis_results": {},
            "expert_confidence_scores": {},
            "expert_evidence": {}

...
from src.nodes.experts.contact_expert import (
...
TOOL_TO_STEP_KEY = {
    "analyze_location_context": "contact_step1_result",
    "analyze_spectral_analysis": "contact_step2_result",
    "analyze_thermal_gradient": "contact_step3_result",
    "analyze_surface_analysis": "contact_step4_result"
}
...
def create_step_tools():
    ...
    """
    @tool
    def analyze_location_context(image_path: str) -> Dict[str, Any]:
        """이미지의 위치적 맥락(접속점 여부, 전선 끝단 등)을 식별합니다."""
        try:
            image_data = _load_image_data(image_path)
            return step1_location_context(image_data, verbose=False)
        except Exception as e:
            return {"error": str(e)}
    
    @tool
    def analyze_spectral_analysis(image_path: str) -> Dict[str, Any]:
        """아산화동(Cu2O) 증식 여부를 판단하기 위해 붉은색/적갈색 패턴을 분석합니다."""
        try:
            image_data = _load_image_data(image_path)
            return step2_spectral_analysis(image_data, verbose=False)
        except Exception as e:
            return {"error": str(e)}
    
    @tool
    def analyze_thermal_gradient(image_path: str) -> Dict[str, Any]:
        """전선의 탄화 패턴을 통해 열적 구배(Thermal Gradient)를 시각화합니다."""
        try:
            image_data = _load_image_data(image_path)
            return step3_thermal_gradient(image_data, verbose=False)
        except Exception as e:
            return {"error": str(e)}
    
    @tool
    def analyze_surface_analysis(image_path: str) -> Dict[str, Any]:
        """금속 표면의 곰보 자국(Pitting)이나 전기적 부식 흔적을 정밀 분석합니다."""
        try:
            image_data = _load_image_data(image_path)
            return step4_surface_analysis(image_data, verbose=False)
        except Exception as e:
            return {"error": str(e)}
    
    return [
        analyze_location_context,
        analyze_spectral_analysis,
        analyze_thermal_gradient,
        analyze_surface_analysis
    ]
...
def create_agent_node(all_tools: List[Any]):
    """ReAct 에이전트 노드 생성"""
    def agent_node(state: ContactExpertState) -> Dict[str, Any]:
...
2. 상황에 따라 필요한 도구를 자유롭게 선택하여 사용하세요. 사용 가능한 분석 도구:
   - analyze_location_context: 위치적 맥락(접속점, 전선 끝단) 식별
   - analyze_spectral_analysis: 아산화동(Cu2O) 증식 여부 판단 (붉은색/적갈색 패턴 분석)
   - analyze_thermal_gradient: 열적 구배 시각화 (탄화 패턴 분석)
   - analyze_surface_analysis: 금속 표면 분석 (곰보 자국, 전기적 부식 흔적)
...
def contact_expert_wrapper_node(state: InvestigationState) -> Dict[str, Any]:
    """
    InvestigationState를 ContactExpertState로 변환하여 ReAct 그래프를 실행하고,
    ...
    try:
        ...
        initial_state: ContactExpertState = {
            "messages": [
                HumanMessage(content=f"이미지를 분석하고 접촉불량 여부를 판단하세요. 이미지 경로: {temp_image_path}")
            ],
        ...
        step_results = {
            "contact_step1_result": {},
            "contact_step2_result": {},
            "contact_step3_result": {},
            "contact_step4_result": {}
        }
        ...
        for msg in messages:
            if isinstance(msg, ToolMessage):
                ...
                if msg.name in TOOL_TO_STEP_KEY:
        ...
        step1_result = step_results["contact_step1_result"]
        step2_result = step_results["contact_step2_result"]
        step3_result = step_results["contact_step3_result"]
        step4_result = step_results["contact_step4_result"]

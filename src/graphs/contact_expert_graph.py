...
def distribute_work(state: ContactExpertState) -> Union[str, List[Send]]:
    return distribute_work_generic(state, "Contact")
...
def _contact_initial_state_factory(temp_image_path, hotspots, preprocessed_hotspots, **kwargs) -> ContactExpertState:
    return {
        "messages": [],
...
contact_expert_wrapper_node = create_expert_wrapper_node(
    expert_name="Contact",
    expert_id="contact",
    build_graph_fn=build_contact_expert_graph,
    default_initial_state_factory=_contact_initial_state_factory
)

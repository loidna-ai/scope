...
from src.nodes.experts.strand_fracture_expert import (
    step1_tip_morphology,
    step2_bead_distribution,
    step3_mechanical_fatigue,
...
# 상수 정의
STEP_TOOLS = [
    "analyze_tip_morphology",
    "analyze_bead_distribution",
    "analyze_fatigue"
]
...
TOOL_TO_STEP_KEY = {
    "analyze_tip_morphology": "strand_fracture_step1_result",
    "analyze_bead_distribution": "strand_fracture_step2_result",
    "analyze_mechanical_fatigue": "strand_fracture_step3_result"
}
...
def create_step_tools():
    ...
    """
    @tool
    def analyze_tip_morphology(image_path: str) -> Dict[str, Any]:
        """소선 끝단의 형상을 분석하여 반단선(Semi-disconnection) 여부를 판단합니다."""
        try:
            image_data = _load_image_data(image_path)
            return step1_tip_morphology(image_data, verbose=False)
        except Exception as e:
            return {"error": str(e)}
    
    @tool
    def analyze_bead_distribution(image_path: str) -> Dict[str, Any]:
        """용융망울의 크기와 분포를 분석하여 반단선 아크 여부를 확인합니다."""
        try:
            image_data = _load_image_data(image_path)
            return step2_bead_distribution(image_data, verbose=False)
        except Exception as e:
            return {"error": str(e)}
    
    @tool
    def analyze_mechanical_fatigue(image_path: str) -> Dict[str, Any]:
        """기계적 피로 흔적을 분석하여 반단선 발생 가능성을 판단합니다."""
        try:
            image_data = _load_image_data(image_path)
            return step3_mechanical_fatigue(image_data, verbose=False)
        except Exception as e:
            return {"error": str(e)}
    ...
    return [
        analyze_tip_morphology,
        analyze_bead_distribution,
        analyze_mechanical_fatigue
    ]


class StrandFractureExpertState(MessagesState):
    """
    StrandFracture Expert ReAct State
    
    MessagesState를 상속받아 messages 필드가 자동으로 포함됩니다.
    """
    strand_fracture_step1_result: Optional[Dict[str, Any]]
    strand_fracture_step2_result: Optional[Dict[str, Any]]
    strand_fracture_step3_result: Optional[Dict[str, Any]]
    image_path: Optional[str]  # 현재 사용 중인 이미지 파일 경로 (표준 패턴)


def create_agent_node(all_tools: List[Any]):
    """ReAct 에이전트 노드 생성"""
    def agent_node(state: StrandFractureExpertState) -> Dict[str, Any]:
...
당신은 전기화재 감식 전문가 'StrandFracture Expert'입니다.
...
2. 상황에 따라 필요한 도구를 자유롭게 선택하여 사용하세요. 사용 가능한 분석 도구:
   - analyze_tip_morphology: 소선 끝단의 형상 분석 (반단선 여부 판단)
   - analyze_bead_distribution: 용융망울의 크기와 분포 분석 (반단선 아크 여부 확인)
   - analyze_mechanical_fatigue: 기계적 피로 흔적 분석 (반단선 발생 가능성 판단)
...
def strand_fracture_expert_wrapper_node(state: InvestigationState) -> Dict[str, Any]:
    """
    InvestigationState를 StrandFractureExpertState로 변환하여 ReAct 그래프를 실행하고,
    ...
    try:
        ...
        if image_data is None:
            return {
                "errors": ["StrandFracture 전문가: 이미지를 추출할 수 없습니다."],
                "expert_reports": [],
        ...
        initial_state: StrandFractureExpertState = {
            "messages": [
                HumanMessage(content=f"이미지를 분석하고 반단선 여부를 판단하세요. 이미지 경로: {temp_image_path}")
            ],
        ...
        step_results = {
            "strand_fracture_step1_result": {},
            "strand_fracture_step2_result": {},
            "strand_fracture_step3_result": {}
        }
        ...
        step1_result = step_results["strand_fracture_step1_result"]
        step2_result = step_results["strand_fracture_step2_result"]
        step3_result = step_results["strand_fracture_step3_result"]
        ...
        traceback.print_exc()
        return {
            "errors": [f"StrandFracture 전문가 ReAct 에이전트 오류: {str(e)}"],
            "expert_reports": [],
            "expert_analysis_results": {},
            "expert_confidence_scores": {},
            "expert_evidence": {}

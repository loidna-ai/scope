"""
Aging 전문가 서브그래프 빌더 (Map-Reduce Pattern with Send API)
"""
import os
...
def distribute_work(state: AgingExpertState) -> Union[str, List[Send]]:
    """Fan-Out to Workers"""
    return distribute_work_generic(state, "Aging")

# ===== Conditional Routing Functions =====

def route_supervisor_decision(state: AgingExpertState) -> Literal["debate", "finalize"]:
    return route_supervisor_decision_generic(state)

def route_verdict_debate(state: AgingExpertState) -> Literal["back_to_analyst", "finalize"]:
    return route_verdict_debate_generic(state)

# ===== Graph Builder =====

def build_aging_expert_graph():
    """Aging 전문가 서브그래프 빌드 - Map-Reduce Pattern"""
...
def _aging_initial_state_factory(temp_image_path, hotspots, preprocessed_hotspots, **kwargs) -> AgingExpertState:
    return {
        "messages": [],
...
aging_expert_wrapper_node = create_expert_wrapper_node(
    expert_name="Aging",
    expert_id="aging",
    build_graph_fn=build_aging_expert_graph,
    default_initial_state_factory=_aging_initial_state_factory
)

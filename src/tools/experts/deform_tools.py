...
식별된 특징의 위치를 나타내는 Bounding Box 좌표(0~1000 정규화 좌표, [ymin, xmin, ymax, xmax])를 포함하여 다음 JSON 형식으로 응답하세요:
{
    ...
    "physical_force_evidence": "물리적 힘의 증거 설명 (자연스러운 흐트러짐인지 강제 변형인지 구분)",
    "confidence": 0-100,
...
식별된 특징의 위치를 나타내는 Bounding Box 좌표(0~1000 정규화 좌표, [ymin, xmin, ymax, xmax])를 포함하여 다음 JSON 형식으로 응답하세요:
{
    ...
    "constraint_evidence": "물리적 구속의 증거 설명 (망울 형상 포함)",
...
def step1_deformation(image_data: bytes, verbose: bool = False) -> Dict[str, Any]:
    """Step 1: 기계적 변형 흔적 분석"""
    if verbose:
    ...
    if verbose:
        deformation_detected = result.get("deformation_detected", False)
...
def calculate_confidence_score(
    ...
    mechanical_deformation_detected = step1_result.get("deformation_detected", False)
...
def collect_evidence(
    ...
    if strand_splaying_detected and (step2_result.get("cut_marks_detected", False) or step2_result.get("crushed_state_detected", False)):
        evidence.append({
            ...
            "details": step2_result.get("physical_force_evidence", "")
        })
    ...
    if bead_confinement_detected:
        ...
        if bead_shape == "flattened":
            evidence.append({
                ...
                "details": step3_result.get("constraint_evidence", "")
            })
        else:
            evidence.append({
                "step": 3,
...
def analyze_mechanical_damage(payload: List[Any], verbose: bool = False) -> Dict[str, Any]:
    ...
    step1_result = step1_deformation(image_data, verbose)

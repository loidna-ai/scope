...
**Step 2. 전선 본체(줄기) 파악 및 맥락 분석 (Wire Context Analysis)**
- Hotspot 영역으로 들어오고 나가는 **주요 전선의 본체(줄기)**가 몇 개인지 시각적으로 파악하십시오.
- **[독립적인 전선의 정의]** 서로 다른 피복 색상, 굵기, 방향성을 가진 별개의 전선을 의미합니다.

**Step 3. 형태 복원 및 매칭 (Reconstruction)**
- 화재로 인한 손상(녹음, 끊어짐)을 감안할 때, 이 잔해의 원래 실루엣은 무엇에 가깝습니까?
    - Terminal (고정 접속부): 나사, 볼트, 너트가 체결된 형태, PCB에 납땜된 핀 헤더
    - Splice (전선 결선부): **서로 다른 2개 이상의 '독립적인 전선'이 의도적으로 결합된 부위.** 나사, 압착 단자, 절연 테이프, 와이어 캡, 꼬임 접속 등으로 묶여 있거나 결합된 형태.
    - Wire (전선/케이블): 직선 또는 완만한 곡선을 그리며 이어지는 연속된 도체. **단일 전선이 끊어져 상·하단 2개의 세그먼트로 나뉜 파단부 역시 1개의 전선이므로 Wire(손상됨)에 해당합니다.**
...
{{
   "deduced_type": "Wire",
   "confidence": 95,
   "visual_description": "전선이 끊어져 위/아래 두 갈래의 세그먼트로 나뉘어 있으며, 파단부의 구리 소선들이 엉켜 있음. 들어오고 나가는 전선의 피복 색상과 굵기가 동일하여 단일 전선의 파단부로 보임.",
   "reasoning": "Step 1: 끊어지고 엉킨 구리 소선들이 관찰됨. Step 2: 위/아래 두 갈래로 보이나, 동일한 전선이 끊어진 것이므로 전선 본체는 1개임. Step 3: 2개 이상의 독립된 전선 결합이 아니므로 Splice가 아니며, 단일 전선의 파단부인 Wire에 해당함. Step 4: 따라서 Wire로 판정함."
}}
</output_schema>
"""

...
- **Image_A와 Image_B를 다음 4가지 구역(Zone)으로 나누어 스캔하고 물리적 속성을 서술하십시오.**
...
<expert_knowledge>
- 최선단(끝단부) 맺힌 비드가 모든 소선을 완벽하게 삼켜서 매끄러운 하나의 큰 덩어리가 되어 있고, 주변에 흩어진 작은 비드나 끊어진 소선 가닥이 전혀 없는 경우 일반 단락으로 판단.
- 연선의 경우, 소선들이 하나로 뭉치지 못하고 흩어지며 각 소선 끝에 개별적인 작은 비드가 확인되는 경우 반단선으로 판단.
- necking은 반단선의 증거이지만, 최종 단락 시 용융되어 사라질 수 있으므로, **'보이지 않는다'고 해서 반단선을 배제해서는 안 됨.**
- 용융 흔적이 없는 경우 반단선 아님으로 판단.
- 최선단(끝단부)의 최대 너비가 기준 도체의 너비와 동일하거나 좁아지는 경우 반단선으로 판단.
</expert_knowledge>

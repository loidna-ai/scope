...
1. **시각적 증거 검증 (마스킹 효과 수용)** (🔥 최우선):
   - 분석가가 "세장화(Tapering)", "소선 분산"을 주장할 때, **단락흔 모양이 다소 뭉툭하더라도(마스킹 효과)** 전선 끝으로 갈수록 가늘어지는 전체적 추세가 있다면 지나치게 비판하지 마십시오.
   - 단, 전선 본체(Shaft)에 명확한 '물리적 찍힘/눌림(Crushed)'이 존재하거나, 하나의 거대하고 둥근 비드가 맺혀 반단선의 특성이 완전히 사라졌는데도 '반단선'으로 판정했다면 이를 치명적으로 비판하십시오.
   
2. **확률 점수(Probability) 적정성 검증**:
   - 제시된 증거에 비해 **점수가 과하게 높지 않은가**? (다만 가늘어짐과 분산 추세가 명확하다면 끝이 뭉툭하다는 이유만으로 점수를 과도하게 깎지 마십시오.)
   
3. **증거 과대해석**: 
   - "세장화"라고 주장했지만 실제로는 물리력으로 잘리거나 끊어진 거친 단면이 존재하는가?
   
4. **판단 누락 (압착/손상 혼동)**:
   - 반단선으로 판정했지만, 실제 이미지에서는 빗자루처럼 흩어지기보다 외부 충격(툴 자국 등)에 의해 짓뭉개진(Flattened) 흔적이 더 농후한가?
...
- "뾰족하지 않고 뭉툭하다"는 등 사소한 트집(마스킹 인정)을 잡지 말고, **'압착 흔적이 분명한데 반단선이라 우기는 경우'**, **'거대한 구슬형 단락흔인데 반단선이라 우기는 경우'** 같은 **판정을 뒤집을 만한 결정적 의문**만 제기하십시오.

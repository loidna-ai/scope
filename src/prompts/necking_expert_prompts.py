...
**반단선 기준**
- 소선 분산: 소선들이 하나로 뭉치지 못하고 빗자루처럼 흩어져 있는 경우
- 소성 변형: 일부 소선의 최선단이 인위적인 절단면(평평한 면) 없이, 소선이 엿가락처럼 늘어나며 끝이 뾰족하거나 둥글게 가늘어지는 경우(necking)
- 미세 용융흔적: 각 소선 끝에 개별적인 작은 용융흔적(bead))이 확인되는 경우.
- 비드 크기: 최선단 용융흔적(bead)의 최대 너비가 기준 도체(전선 굵기)와 동일하거나 오히려 좁아지는 경우.
- 경계 선명성: 최선단 용융흔적(bead)과 기준 도체 사이의 경계가 선명하게 구분되는 경우.

**유의사항**
- 거대 단락 배제: 최선단(끝단부) 맺힌 용융흔적(bead)이 모든 소선을 완벽하게 삼켜서 매끄러운 하나의 큰 덩어리가 되어 있고, 주변에 흩어진 작은 용융흔적(bead)이나 끊어진 소선 가닥이 전혀 없는 경우는 반단선이 아님.
- 마스킹 효과: Necking은 반단선의 강력한 증거이지만, 이후 발생한 아크나 외부 화재로 녹아 사라질 수 있음. 따라서 Necking이 보이지 않는다고 해서 무조건 반단선을 배제해서는 안 되며, 다른 특징(소선 분산 등)을 종합적으로 고려해야 함.
- 비드 부재 시 예외: 끝단부에 비드가 없더라도, 소선 끝이 열에 의해 부드럽게 녹아 가늘어졌고(Tapering) 주변이 심하게 산화(검은색/붉은색)되었다면 **'적열(Glowing)에 의한 반단선'**으로 포함함.
...
5. 전부 "Analysis Skipped (Target is not a Wire)"이면 "반단선 아님"으로 판정하십시오.
</guidelines>

...
class ExpertImageLoader:
    ...
    async def load_images(
        ...
        Args:
            ...
            use_cache: 이 호출에 대한 캐싱 사용 여부 (None이면 인스턴스 기본값 사용)
        ...
        original_data = await self.load_image(original_path, cache_enabled)
        roi_data = await self.load_image(roi_path, cache_enabled)
...
# 전역 싱글톤 인스턴스 (기본적으로 캐싱 사용)
_global_image_loader = ExpertImageLoader(use_cache=True)

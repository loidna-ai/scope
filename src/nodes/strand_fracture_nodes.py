"""
Strand Fracture 전문가 노드 및 ReAct 에이전트 정의
"""
...
from src.prompts.strand_fracture_expert_prompts import (
...
class StrandFractureExpertState(MessagesState):
    """
    Strand Fracture Expert ReAct State
    """
...
def build_step1_react_agent(image_path: str):
    ...
    @tool
    def analyze_tip_morphology_internal(image_path: str) -> str:
        """이미지에서 소선의 끝단 형상, 뾰족함, 네킹 현상을 분석합니다. 이미지 보정이 필요하면 먼저 보정 도구를 사용하세요."""
        try:
            image_data = _load_image_data(image_path)
            result = step1_tip_morphology(image_data, verbose=False)
...
def build_step2_react_agent(image_path: str):
    ...
    @tool
    def analyze_bead_distribution_internal(image_path: str) -> str:
        """이미지에서 용융망울의 크기와 분포(미세 망울 여부)를 분석합니다. 이미지 보정이 필요하면 먼저 보정 도구를 사용하세요."""
        try:
            image_data = _load_image_data(image_path)
            result = step2_bead_distribution(image_data, verbose=False)
            return json.dumps(result, ensure_ascii=False) if isinstance(result, dict) else str(result)
        except Exception as e:
            return json.dumps({"error": str(e)}, ensure_ascii=False)
...
def build_step3_react_agent(image_path: str):
    ...
    @tool
    def analyze_mechanical_fatigue_internal(image_path: str) -> str:
        """이미지에서 기계적 피로 흔적(반복 굽힘, 비틀림 등)을 분석합니다. 이미지 보정이 필요하면 먼저 보정 도구를 사용하세요."""
        try:
            image_data = _load_image_data(image_path)
            result = step3_mechanical_fatigue(image_data, verbose=False)
            return json.dumps(result, ensure_ascii=False) if isinstance(result, dict) else str(result)
        except Exception as e:
            return json.dumps({"error": str(e)}, ensure_ascii=False)
...
def step1_node(state: StrandFractureExpertState):
    ...
    for msg in reversed(step_messages):
        if isinstance(msg, ToolMessage) and msg.name == "analyze_tip_morphology_internal":
            try:
    ...
    return {
        ...
        "strand_fracture_step1_result": step_result,
...
def step2_node(state: StrandFractureExpertState):
    ...
    step_result = None
    for msg in reversed(step_messages):
        if isinstance(msg, ToolMessage) and msg.name == "analyze_bead_distribution_internal":
            try:
                step_result = json.loads(msg.content)
                break
            except:
    ...
    return {
        "messages": step_messages,
        "strand_fracture_step2_result": step_result,
        "image_path": updated_image_path
    }
...
def step3_node(state: StrandFractureExpertState):
    ...
    step_result = None
    for msg in reversed(step_messages):
        if isinstance(msg, ToolMessage) and msg.name == "analyze_mechanical_fatigue_internal":
            try:
                step_result = json.loads(msg.content)
                break
            except:
    ...
    return {
        "messages": step_messages,
        "strand_fracture_step3_result": step_result,
        "image_path": updated_image_path
    }

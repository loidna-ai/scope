...
async def hotspot_detector_node(state: InvestigationState) -> Dict[str, Any]:
    ...
    try:
        ...
        # Generator를 리스트로 변환하지 않고 바로 소비하거나, 필요시 리스트로 변환
        # 여기서는 asyncio.gather를 위해 task 생성을 위한 순회가 필요함
        # slice_image는 이제 Generator를 반환함
        ...
        # 2. Parallel API Execution setup
        ...
        async def _process_patch(patch_data):
            ...
            # [Optimization] 불필요한 강제 지연 제거 (Rate Limiter가 제어함)
            # await asyncio.sleep(config.API_CALL_DELAY) 
            
            return (patch_data, hotspots) # patch_data와 결과 함께 반환

        # Create tasks from generator
        tasks = []
        for p in patch_generator:
            tasks.append(_process_patch(p))
            patch_stats["total"] += 1
        ...
        for patch_info, patch_hotspots in results:

...
async def _collect_evidence(
    ...
    try:
        ...
        # Pydantic 안전 파싱
        evidence_result = NeckingEvidenceResult.model_validate_json(response.text)
        ...
        if hasattr(evidence_result, 'step5_extracted_evidence'):
            ...
            severity_score = (hotspot or {}).get("severity_score", 50)  # 탐지 단계의 심각도 점수 유지 (v0.5.3)
            is_critical = False
...
async def analyze_hotspot_worker(state: WorkerState) -> Dict[str, List[Dict]]:
    ...
    try:
        ...
        evidence_data = await _collect_evidence(hotspot_id, connection_type, roi_image_path, image_path, hotspot=hotspot)

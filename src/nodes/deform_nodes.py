...
from src.prompts.deform_expert_prompts import (
    ...
    get_deform_plug_prompt,
    get_final_verdict_prompt
)

# --- State Definition ---
class DeformExpertState(TypedDict):
    """Deform 전문가 그래프 상태"""
    # 기본 메시지 상태
    messages: List[Any]
...
def roi_crop_node(state: DeformExpertState) -> Dict[str, Any]:
...
def component_classifier_node(state: DeformExpertState) -> Dict[str, Any]:
...
def deform_wire_node(state: DeformExpertState) -> Dict[str, Any]:
    """Node 3A: Deform Wire Analysis (Mechanical Compression)"""
    print(f"\n⚡ [Deform] Wire Analysis (Dual Input: Context + Detail)")
    roi_path = state.get("roi_image_path")
    original_image_path = state.get("image_path")
    
    prompt = get_deform_wire_prompt(roi_path)
...
def deform_plug_node(state: DeformExpertState) -> Dict[str, Any]:
    """Node 3B: Deform Plug Analysis (Inward Crushing)"""
    print(f"\n⚡ [Deform] Plug Analysis (Dual Input: Context + Detail)")
    roi_path = state.get("roi_image_path")
    original_image_path = state.get("image_path")
    
    prompt = get_deform_plug_prompt(roi_path)
...
def result_aggregator_node(state: DeformExpertState) -> Dict[str, Any]:
    """Loop End: 결과 집계"""
    entry = {
...
def verdict_node(state: DeformExpertState) -> Dict[str, Any]:
    """Step 4: Final Verdict"""
    print("--- [Deform] Node 4: Final Verdict ---")
    ...
    final_report = f"""[Deform 전문가 최종 판정]

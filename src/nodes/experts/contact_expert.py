...
def step1_location_context(image_part: Part, verbose: bool = False) -> Dict[str, Any]:
    """Step 1: 위치적 맥락 확인"""
    if verbose:
        print("\n🔍 [Step 1] 위치적 맥락 확인 시작...")
...
def step3_thermal_gradient(image_part: Part, verbose: bool = False) -> Dict[str, Any]:
    """Step 3: 열적 구배 분석"""
    if verbose:
        print("\n🔥 [Step 3] 열적 구배 분석 시작...")
...
def step4_surface_analysis(image_part: Part, verbose: bool = False) -> Dict[str, Any]:
    """Step 4: 금속 표면 상태 분석"""
    if verbose:
        print("\n🔬 [Step 4] 금속 표면 상태 분석 시작...")
    
    response_text, thinking_info = call_gemini_vision(STEP4_PROMPT, image_part, "Step 4", verbose)
    result = parse_json_response(response_text)
...
def analyze_connection_failure(payload: List[Any], verbose: bool = False) -> Dict[str, Any]:
    ...
    # 4단계 순차 분석 실행
    step1_result = step1_location_context(image_part, verbose)
    step2_result = step2_spectral_analysis(image_part, verbose)
    step3_result = step3_thermal_gradient(image_part, verbose)
    step4_result = step4_surface_analysis(image_part, verbose)
